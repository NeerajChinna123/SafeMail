...
export const ImageWithBoundingBox3 = (props: any) => {
    ...
    const boxStyle1 = {
        ...
        // Using the first prediction for demonstration
         //@ts-ignore
        left: props?.data[2]?.apiCallresponse?.predictions[0]?.x / process.env.NEXT_PUBLIC_B3_BS2_left + 'px',
         //@ts-ignore
        top: props?.data[2]?.apiCallresponse?.predictions[0]?.y / process.env.NEXT_PUBLIC_B3_BS2_top + 'px',
         //@ts-ignore
        width: props?.data[2]?.apiCallresponse?.predictions[0]?.width / process.env.NEXT_PUBLIC_B3_BS2_width + 'px',
         //@ts-ignore
        height: props?.data[2]?.apiCallresponse?.predictions[0]?.height / process.env.NEXT_PUBLIC_B3_BS2_height + 'px'
    };
    ...
    return (
        <>
            <div style={{ position: 'relative', width: props?.data[0]?.apiCallresponse?.image.width / 3, height: props?.data[0]?.apiCallresponse?.image.height / 3 }}>
                <img src={props?.data[2]?.image} alt="Analyzed" />
                 {/* @ts-ignore */}
